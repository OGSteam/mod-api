...
//find ne fonctionnera pas (3.3.3 ->3.4)
...
class Universe_Model
{
    ...
    /**
     * Obtiens la liste des planètes qui occupe la galaxie
     * attention, ne retourne pas de "row" vide (voir get_system)
     * @param $galaxy integer Galaxie
     * @return array
     */
    public function get_galaxy_occupation($galaxy)
    {
        global $db;
        ...
        if ($db->sql_numrows($result) > 0) {
            ...
            {
                $retour = array();
                $retour['galaxy']= $row['galaxy'];
                $retour['system'] = $row['system'];
                $retour['row'] = $row['row'];
                $retour['name'] = $row['name'];
                $retour['ally'] = $row['ally'];
                $retour['player'] = $row['player'];
                $retour['moon'] = $row['moon'];
                $retour['phalanx']= $row['phalanx'];
                $retour['gate']= $row['gate'];
                $retour['last_update_moon']= $row['last_update_moon'];
                $retour['status']= $row['status'];
                $retour['last_update'] = $row['last_update'];
                $retour['user_name'] = $row['user_name'];
                $population[] = $retour;
            }
        }
        return $population;
