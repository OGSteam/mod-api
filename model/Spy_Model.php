...
class Spy_Model
{
    ...
    /**
     * @param $since temps en seconde
     * @return array
     *
     * Todo = choix du sender id (=$user_id)
     */
    public function get_SpySince($since) {
    global $db;
    $favorite = array();
    $request = "select " . TABLE_PARSEDSPY .".*,  " . TABLE_UNIVERSE . ".moon, " . TABLE_UNIVERSE . ".ally, " . TABLE_UNIVERSE . ".player, " . TABLE_UNIVERSE . ".status";
    $request .= " from " . TABLE_PARSEDSPY . ", " . TABLE_UNIVERSE;
    $request .= " where " . TABLE_PARSEDSPY . ".dateRE > " . $since . " and CONCAT(" . TABLE_UNIVERSE . ".galaxy,':'," . TABLE_UNIVERSE . ".system,':'," . TABLE_UNIVERSE . ".row)=" . TABLE_PARSEDSPY .".coordinates";
    $tResult=array();
